...
var AddressLookupTableAccount = class {
  ...
  isActive() {
    const U64_MAX2 = BigInt("0xffffffffffffffff");
    return this.state.deactivationSlot === U64_MAX2;
  }
...
// .build/client/math.js
var SCALE = 1n << 64n;
var U128_MAX = (1n << 128n) - 1n;
var U64_MAX = (1n << 64n) - 1n;
function activeSlot(nowTs, startTs, rotateSecs, n) {
...
// .build/client/base58.js
var ALPHABET = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
var MAP = {};
for (let i2 = 0; i2 < ALPHABET.length; i2++)
  MAP[ALPHABET[i2]] = i2;

// .build/client/layout.js
var POOL_SEED = "pool";
var SOL_VAULT_SEED = "sol_vault";

// .build/client/pda.js
function poolAddress(programId, mint) {
  return PublicKey.findProgramAddressSync([Buffer.from(POOL_SEED), mint.toBuffer()], programId)[0];
}
function solVaultAddress(programId, mint) {
  return PublicKey.findProgramAddressSync([Buffer.from(SOL_VAULT_SEED), mint.toBuffer()], programId)[0];
}

// .build/client/instructions.js
...
var Side = { BuyX: 0, SellX: 1 };
function u642(value) {
  const b = new Uint8Array(8);
  new DataView(b.buffer).setBigUint64(0, value, true);
  return b;
}
function feedsFor(pool, nowTs) {
  const scheduled = activeSlot(BigInt(Math.floor(nowTs)), pool.startTs, pool.rotateSecs, pool.nStocks);
  return {
    tracked: new PublicKey(pool.stockFeeds[pool.active]),
    current: new PublicKey(pool.stockFeeds[scheduled])
  };
}
function swapInstruction(p) {
  const now = p.nowTs ?? Math.floor(Date.now() / 1e3);
  const { tracked, current } = feedsFor(p.pool, now);
  const mint = new PublicKey(p.pool.xMint);
  const data = new Uint8Array(18);
  data[0] = Tag.Swap;
  data[1] = p.side;
  data.set(u642(p.amountIn), 2);
  data.set(u642(p.minOut), 10);
  return new TransactionInstruction({
    programId: p.programId,
    data: Buffer.from(data),
    keys: [
      { pubkey: poolAddress(p.programId, mint), isSigner: false, isWritable: true },
      { pubkey: p.user, isSigner: true, isWritable: true },
      { pubkey: p.userX, isSigner: false, isWritable: true },
      { pubkey: new PublicKey(p.pool.xVault), isSigner: false, isWritable: true },
      { pubkey: solVaultAddress(p.programId, mint), isSigner: false, isWritable: true },
      { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
      { pubkey: SystemProgram.programId, isSigner: false, isWritable: false },
      { pubkey: new PublicKey(p.pool.solFeed), isSigner: false, isWritable: false },
      { pubkey: tracked, isSigner: false, isWritable: false },
      { pubkey: current, isSigner: false, isWritable: false }
    ]
  });
}
...
async function sendSwap({ rpcUrl, programId, pool, side, amountIn, minOut, nowTs }) {
  const wallet = getWallet();
